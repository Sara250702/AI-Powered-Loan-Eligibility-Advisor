...
genai.configure(api_key='AIzaSyA-2tQBjcxtqvsxAv5jqQf2nM_y4Yq4vYI')
def initialize_session_start():
...
def processing_data(gender , married , dependents , education , employed , credit , area , ApplicantIncome, CoapplicantIncome, LoanAmount , Loan_Amount_Term):
    try:
...
def show_chatbot():
   ...
   st.markdown("""Chat with our AI- powered chatbot to get personalised loan prediction. Simply answer the question.""")

   initialize_session_start()
                ...
                with st.chat_message("assistant"):
                    with st.spinner("Thinking..."):
                        try:
                            ...
                            features = processing_data(gender, married, dependents, education, self_employed, credit_history, property_area, applicant_income, coapplicant_income, loan_amount, loan_amount_term)
